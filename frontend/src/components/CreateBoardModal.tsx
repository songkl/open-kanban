...
function BoardForm({ editingBoard, templates, onClose, onSubmit, t }: BoardFormProps) {
  ...
  // Default to public to match the backend's DEFAULT 1 — existing
  // API clients that omit isPublic keep their previous behaviour.
  const [isPublic, setIsPublic] = useState(editingBoard?.isPublic ?? true);
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-5">
      ...
      <div>
          ...
          <button
            ...
            className={`flex items-center justify-center gap-2 rounded-xl border px-4 py-3 text-sm font-medium transition-all ${
              isPublic
                ...
                : 'border-zinc-200 dark:border-zinc-700 bg-zinc-50 dark:bg-zinc-700 text-zinc-600 dark:text-zinc-300 hover:bg-zinc-100 dark:hover:bg-zinc-600'
            }`}
          >
        ...
        <p className="mt-1.5 text-xs text-zinc-400 dark:text-zinc-500">
          {isPublic ? t('board.publicHint') : t('board.privateHint')}
        </p>
      </div>
...
export function CreateBoardModal({
  ...
  return (
    ...
    >
      ...
      <div
        className="relative z-10 w-full max-w-md rounded-2xl bg-white dark:bg-zinc-800 border border-zinc-100 p-6"
