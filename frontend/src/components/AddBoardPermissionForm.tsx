...
interface AddBoardPermissionFormProps {
  boardId: string;
  onPermissionAdded?: () => void;
}

export function AddBoardPermissionForm({ boardId, onPermissionAdded }: AddBoardPermissionFormProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setError(null);
    try {
      await authApi.setPermission(selectedUserId, boardId, selectedAccess);
      setSelectedUserId('');
      setSelectedAccess('READ');
      onPermissionAdded?.();
    } catch (err) {
      console.error('Failed to set board permission:', err);
      setError(t('board.permissionAddFailed'));
    } finally {
      setLoading(false);
    }
  };
    ...
    <form onSubmit={handleSubmit} className="space-y-3">
      ...
      <div className="flex gap-2">
        <select
          ...
          required
        >
          <option value="">{t('board.selectUser')}</option>
          {users.map((user) => (
        ...
        >
          {loading ? t('common.loading') : t('board.add')}
        </button>
      </div>
    </form>
  );
