...
const SEED_BOARD_ID = "b-tasks-complete";
...
describe("CLI `kanban tasks complete` e2e (against a real Go server)", () => {
  ...
  beforeEach(() => {
    workDir = mkdtempSync(join(tmpdir(), "kanban-tasks-complete-"));
    xdgHome = mkdtempSync(join(tmpdir(), "kanban-tasks-complete-xdg-"));
  });
  ...
  it(
    ...
    async () => {
      ...
      console.log("kanban tasks complete stdout:\n" + result.stdout);
      // eslint-disable-next-line no-console
      console.log("kanban tasks complete stderr:\n" + result.stderr);
      // eslint-disable-next-line no-console
  ...
  it(
    ...
    async () => {
      ...
      const expectedColumns = [DOING_COLUMN_ID, REVIEW_COLUMN_ID, DONE_COLUMN_ID];
      for (const expected of expectedColumns) {
        ...
        expect(
          result.code,
          `expected exit 0 advancing to ${expected}, got ${result.code}.\nstdout:\n${result.stdout}\nstderr:\n${result.stderr}`
        ).toBe(0);
...
describe("tasks-complete e2e prerequisites", () => {
