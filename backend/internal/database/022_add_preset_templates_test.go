...
func TestSQLiteMigration022_AddsPresetTemplates(t *testing.T) {
	...
	// Bring the schema up to the state JUST BEFORE migration 22 so the
	// Migrate(22) call exercises only the preset-templates addition.
...
// TestSQLiteMigration022_DownDropsPresetTemplates asserts the down
...
func TestSQLiteMigration022_DownDropsPresetTemplates(t *testing.T) {
	db, err := sql.Open("sqlite3", "file::memory:?cache=shared")
	if err != nil {
		t.Fatalf("failed to open test db: %v", err)
	}
	...
	if err := m.Migrate(22); err != nil && err != migrate.ErrNoChange {
		t.Fatalf("failed to migrate to v22: %v", err)
	}

	if err := m.Migrate(21); err != nil && err != migrate.ErrNoChange {
		t.Fatalf("failed to roll back to v21: %v", err)
	}
