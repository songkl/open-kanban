...
// TestSQLiteMigration024_AddsUserNotificationPreferences exercises
// the per-user notification preferences migration (s-1203) end-to-end
// and asserts the contracts the spec calls out:
//
//   - The user_notification_preferences table exists after migration 24.
...
func TestSQLiteMigration024_AddsUserNotificationPreferences(t *testing.T) {
...
// TestSQLiteMigration024_DownDropsUserNotificationPreferences asserts
...
func TestSQLiteMigration024_DownDropsUserNotificationPreferences(t *testing.T) {
	db, err := sql.Open("sqlite3", "file::memory:?cache=shared")
	if err != nil {
		t.Fatalf("failed to open test db: %v", err)
	}
	...
	if err := m.Migrate(24); err != nil && err != migrate.ErrNoChange {
		t.Fatalf("failed to migrate to v24: %v", err)
	}

	if err := m.Migrate(23); err != nil && err != migrate.ErrNoChange {
		t.Fatalf("failed to roll back to v23: %v", err)
	}
