...
func TestSQLiteMigration009_AddsNotificationsTable(t *testing.T) {
	...
	// Bring the schema up to the state JUST BEFORE migration 9 so
	// the next Migrate(9) call exercises only the notifications
	// addition.
...
// TestSQLiteMigration009_DownDropsNotificationsTable asserts that
...
func TestSQLiteMigration009_DownDropsNotificationsTable(t *testing.T) {
	db, err := sql.Open("sqlite3", "file::memory:?cache=shared")
	if err != nil {
		t.Fatalf("failed to open test db: %v", err)
	}
