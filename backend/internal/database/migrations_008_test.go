...
// TestSQLiteMigration008_PreservesRowCount exercises the audit-columns
...
// and column_permissions rows, then runs migration 8 and verifies the
// row counts and the audit column defaults.
func TestSQLiteMigration008_PreservesRowCount(t *testing.T) {
	...
	// Bring the schema up to the state JUST BEFORE migration 8 so
	// the next Migrate(8) call exercises only the audit-columns
	// addition. Migrations 1..7 are the prior baseline; migration
	// 8 is the one we want to assert on.
	...
	// Capture row counts BEFORE running migration 8.
	...
	// Apply migration 8: this is the one that adds
	...
	// Row counts must be identical: migration 8 is supposed to be
	...
	for _, idx := range wantIndexes {
		...
		if n == 0 {
			t.Errorf("expected index %s to exist after migration 8", idx)
		}
	}
}

// TestSQLiteMigration008_RoundTripDown verifies the down migration
...
func TestSQLiteMigration008_RoundTripDown(t *testing.T) {
	db, err := sql.Open("sqlite3", "file::memory:?cache=shared")
	if err != nil {
		t.Fatalf("failed to open test db: %v", err)
	}
