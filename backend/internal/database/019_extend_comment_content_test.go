...
// TestSQLiteMigration019_AcceptsLongContent pins down the contract of
...
// handler layer).
func TestSQLiteMigration019_AcceptsLongContent(t *testing.T) {
...
// TestSQLiteMigration019_PreservesExistingComments verifies that the
...
func TestSQLiteMigration019_PreservesExistingComments(t *testing.T) {
	db, err := sql.Open("sqlite3", "file::memory:?cache=shared")
	if err != nil {
		t.Fatalf("failed to open test db: %v", err)
	}
	...
	// documented no-op (see sqlite/019_extend_comment_content.down.sql),
