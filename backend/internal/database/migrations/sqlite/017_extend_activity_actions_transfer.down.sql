-- Reverse of 005_extend_activity_actions_transfer.up.sql: rebuild
-- the activities table with the prior CHECK list (the one baked
-- into 002_extend_activity_actions.up.sql).
--
-- Because the up migration widened the constraint, the down
-- migration has to drop any rows whose action is no longer
-- permitted (PERMISSION_TRANSFER) — otherwise the INSERT INTO
...
INSERT INTO activities_old
    ...
    WHERE action NOT IN ('PERMISSION_TRANSFER');
