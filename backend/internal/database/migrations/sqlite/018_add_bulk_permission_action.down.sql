-- Reverse of 006_add_bulk_permission_action.up.sql: rebuild the
-- activities table with the prior CHECK list (the one baked into
-- 005_extend_activity_actions_transfer.up.sql).
--
-- Because the up migration widened the constraint, the down
-- migration has to drop any rows whose action is no longer
-- permitted (PERMISSION_BULK_GRANT) — otherwise the INSERT INTO
-- activities_old … SELECT would violate the strict CHECK. This is
-- intentional: rolling back a schema change is allowed to be lossy.
...
INSERT INTO activities_old
    ...
    WHERE action NOT IN ('PERMISSION_BULK_GRANT');
