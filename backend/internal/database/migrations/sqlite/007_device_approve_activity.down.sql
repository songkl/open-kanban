-- Reverse of 007_device_approve_activity.up.sql: rebuild the
-- activities table with the previous CHECK lists (the ones baked into
-- 002_extend_activity_actions.up.sql).
--
-- Because the up migration widened both constraints, the down
-- migration has to drop any rows whose action or target_type is no
-- longer permitted (DEVICE_APPROVE / DEVICE) — otherwise the
...
INSERT INTO activities_old
    ...
    WHERE action <> 'DEVICE_APPROVE' AND target_type <> 'DEVICE';
