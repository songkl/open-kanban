-- Reverse of 015_add_column_bulk_actions.up.sql: rebuild the
-- activities table with the prior CHECK list (the one baked into
-- 006_add_bulk_permission_action.up.sql).
--
-- Because the up migration widened the constraint, the down
-- migration has to drop any rows whose action is no longer
-- permitted (BULK_ARCHIVE_COLUMN, BULK_COMPLETE_COLUMN) — otherwise
-- the INSERT INTO activities_old … SELECT would violate the strict
...
INSERT INTO activities_old
    ...
    WHERE action NOT IN ('BULK_ARCHIVE_COLUMN', 'BULK_COMPLETE_COLUMN');
