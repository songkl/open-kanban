// Package services — webhook_e2e_test.go is the end-to-end test
// described in plan §5.2 / task s-1147. It exercises the full
// EventCenter pipeline against a live httptest.Server receiver:
//
...
//     (returns 200 OK on every POST).
//  3. Insert one webhook row pointing at that receiver, secret
//     known to the test.
...
//     webhook_deliveries row transitioned to SUCCESS, and the
//     parent webhook's last_success_at was stamped.
//
// A second case exercises the failure path:
//  1. httptest.Server returns 500.
//  2. Webhook is configured with max_retries = 2.
//  3. Publish + drive the retry sweeper until the row
//     transitions to EXHAUSTED.
//  4. Assert: attempt counter exceeded max_retries and the
//     row's status = EXHAUSTED.
//
// The test intentionally uses only the public services.* API
// (NewChannelEventBus, NewEventCenter, NewDefaultDeliverFunc,
// NewRetrySweeper, Publish, SweepOnce). The wire format is the
// production one — the openssl verification vector below is
// hand-computed so a drift in the signing scheme fails loudly
// without needing an external openssl binary.
...
// setupE2EDB spins up an in-memory SQLite with the migration-012
// schema so the EventCenter dispatcher + retry sweeper can run
// against it without touching the filesystem.
func setupE2EDB(t *testing.T) *sql.DB {
	t.Helper()
	db, err := sql.Open("sqlite3", "file:webhook_e2e.db?mode=memory&cache=shared")
	if err != nil {
...
// e2eSchema is the production migration-012 schema, kept in
// this file so the E2E test is self-contained — no test-setup
...
// seedWebhookE2E inserts a single webhook row for the E2E test.
// secretPlain is the hex-encoded secret the test remembers so it
// can re-compute the expected HMAC signature.
func seedWebhookE2E(t *testing.T, db *sql.DB, id, url, secretPlain string, maxRetries int) {
	t.Helper()
	secretBlob, err := hex.DecodeString(secretPlain)
	if err != nil {
...
// its status column reaches one of the supplied statuses or the
// deadline expires.
...
//  2. EventCenter dispatcher matches it, enqueues a delivery job.
...
// opensslLikeHMAC below for the exact formula.
func TestWebhookE2E_HappyPath_ReceiverGetsSignedPOST(t *testing.T) {
	...
	// The EXHAUSTED test below exercises the full sweeper; the
	// happy-path test doesn't need one (the deliverer's first
	// try succeeds), so we instantiate and immediately stop a
	// no-op sweeper to keep the fixture shape consistent.
	noopSweeper := services.NewRetrySweeper(db, bus, nil, time.Second)
	_ = noopSweeper
	noopSweeper.Stop()
	...
	// Verify the HMAC signature matches the openssl
	// incantation:
	//
	...
	// The secret is stored as a BLOB of raw bytes; re-decode
	// from hex so the openssl re-computation uses the same key
	// material the production code used.
	...
	// webhook_deliveries should be SUCCESS and webhooks.last_success_at stamped.
...
// TestWebhookE2E_FailurePath_ExhaustedAfterMaxRetries drives the
// retry pipeline end-to-end. The receiver always returns 500;
// the test publishes one event, then calls SweepOnce repeatedly
// (manually advancing next_retry_at so each iteration is
// immediately due) until the sweeper transitions the row to
// EXHAUSTED.
//
// This exercises:
//   - the deliverer's FAILED + next_retry_at path
//   - markExhausted's attempt > max_retries guard
//   - the full EventCenter → deliverer → sweeper → EXHAUSTED round-trip
func TestWebhookE2E_FailurePath_ExhaustedAfterMaxRetries(t *testing.T) {
	...
	// The retry sweeper hands re-enqueued jobs back through
	// EventCenter.Requeue so the worker pool picks them up.
	...
	// Wait for the first failure (HTTP 500 → FAILED).
	...
	// Drive the sweeper until the row reaches EXHAUSTED. We
	// keep advancing next_retry_at to the past between ticks
	// so each SweepOnce is due to run. The deliverer's HTTP
	// round-trip is short (loopback httptest.Server) so the
	// FAILED → requeue → deliver → FAILED loop completes
	// quickly under the 50ms sleep.
	deadline := time.Now().Add(10 * time.Second)
	lastAttempt := -1
	for time.Now().Before(deadline) {
		// Force the next_retry_at to now so the sweeper picks
		...
		// Bound the per-tick wait so a stuck sweeper surfaces
		// quickly.
	...
	// Receiver should have observed at least maxRetries+1 POSTs
	// (initial + retries) before the EXHAUSTED transition.
	calls := rec.snapshot()
...
// opensslLikeHMAC is declared in webhook_delivery_test.go (same
// package); the e2e test reuses it to keep the openssl
// incantation in one place.
