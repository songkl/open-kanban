...
func fillWebhookHealth(db *sql.DB, out *WebhookHealth, _ time.Time) {
	...
	if err := db.QueryRow(`SELECT COUNT(*) FROM webhooks WHERE enabled = 1`).Scan(&enabledCount); err == nil {
		out.Enabled = enabledCount > 0
	} else {
		slog.Debug("status: webhook enabled count failed", "error", err)
	}
